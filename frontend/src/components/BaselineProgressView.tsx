...
export const BaselineProgressView: React.FC<BaselineProgressViewProps> = () => {
  ...
  const [activities, setActivities] = useState([
    ...
    { id: 3, name: 'Unpack & Mount Workstations', milestone: '4. Equipment Installation', progress: 25, weight: 35.0 }
  ]);
          ...
          <Box className="enterprise-card" sx={{ p: 3, height: '100%' }}>
            ...
            <Box sx={{ p: 2.5, borderRadius: '12px', background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              ...
              <Typography variant="caption" sx={{ color: '#475569' }}>
                Based on total weight sum of <strong>{totalWeight}%</strong> across {activities.length} activities.
              </Typography>
            </Box>
          </Box>
        </Grid>
              ...
              {activities.map((act) => {
                ...
                return (
                  <TableRow key={act.id}>
                    ...
                    <TableCell>
                      <Chip label={`${act.progress}%`} size="small" sx={{ fontWeight: 700 }} />
                    </TableCell>
                    <TableCell>
                      <TextField
                        type="number"
